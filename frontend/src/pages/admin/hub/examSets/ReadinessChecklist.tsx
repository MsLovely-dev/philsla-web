...
function RowIcon({ result }: { result: string }) {
  const status = result.toUpperCase();
  if (status === 'PASSED') return <CheckCircle className="h-4 w-4 shrink-0" />;
  if (status === 'FAILED') return <AlertTriangle className="h-4 w-4 shrink-0" />;
...
export function ReadinessChecklist({ results }: ReadinessChecklistProps) {
  ...
  const passedCount = results.filter((result) => result.result.toUpperCase() === 'PASSED').length;
