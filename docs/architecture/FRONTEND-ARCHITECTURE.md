...
Service contracts under `src/services/` define the frontend boundary for authentication, application submission, reviewer decisions, and shared service responses. `LocalStorageAuthService` preserves the current prototype session behavior behind the `AuthService` contract. `BackendAuthService` can be enabled with `VITE_AUTH_SERVICE_MODE=backend` to call the implemented Django auth boundary endpoints through `VITE_BACKEND_API_BASE_URL`. Mock implementations remain the default and must not be treated as production API behavior. Frontend role values, local storage sessions, and route-level allowed-role lists must never be sent to the backend as authorization evidence. Backend adapters must follow the approved contracts in [API endpoints](../api/API-ENDPOINTS.md); unresolved backend behavior remains `TBD`.
