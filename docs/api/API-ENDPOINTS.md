...
| `GET`, `POST` | `/api/v1/universities/` | Required bearer access token | `SYSTEM_ADMIN`, `UNIVERSITY_ADMIN`, or `ADMISSIONS_REVIEWER` | List the university registry with per-record `courseCount`, or create a university (server-assigned `UNI-#####` code) | Implemented |
...
### `GET, POST /api/v1/universities/`

`GET` returns the full university registry as a plain JSON array (no pagination), each row annotated with its live `courseCount`. `POST` creates a university from `{classification, name, region, city, presidentRector, email, phone, establishedYear, status}`; the `code` is server-assigned as the next sequential `UNI-#####` and is read-only. `name` is required and must be unique within its `region` (case-insensitive) — names may repeat across regions. `establishedYear` is optional and nullable. Each record has the shape `{id, code, classification, name, region, city, presidentRector, email, phone, establishedYear, status, courseCount, createdAt, updatedAt}` with an integer `id`. This endpoint returns institutional directory data only; it does not return student, applicant, account, or assessment data.

### `GET, PUT, PATCH, DELETE /api/v1/universities/{universityId}/`

`GET` returns one university in the same shape as a collection result, including the current `courseCount`. `PUT`/`PATCH` accept the writable create fields (`code` and `courseCount` are read-only) and re-check per-region name uniqueness. `DELETE` returns `204 No Content` and cascade-deletes the university's college courses in the same database transaction. An unknown integer identifier returns `404 NOT_FOUND`. There is no optimistic-concurrency/version control — last write wins.

### `GET, POST /api/v1/universities/{universityId}/courses/`

`GET` returns the path university's college courses as a plain JSON array. `POST` creates a course from `{collegeName, programCode, programName, degreeType, majorSpecialization, durationYears, totalUnits, cutoffPercentile, status}`. The path supplies the authoritative university; `universityId` and `universityCode` are read-only in the response and a client cannot move a course by submitting another identifier. `programName` is required and `programCode` must be unique within the university (case-insensitive). `durationYears` is ≥ 1, `totalUnits` is ≥ 0, and `cutoffPercentile` is a float ≥ 0. Each record has the shape `{id, universityId, universityCode, collegeName, programCode, programName, degreeType, majorSpecialization, durationYears, totalUnits, cutoffPercentile, status, createdAt, updatedAt}`.
...
**Known gap (ADR-010):** unlike the superseded `configuration` registry, these endpoints do **not** yet enforce per-`UNIVERSITY_ADMIN` object scoping ("assigned university boundaries") or module-38 `MOD_38_*` structured permissions — any of the three roles has full CRUD across all universities, and all three can `DELETE`. The `scopes.universityIds` value that scoping would read is not populated anywhere in the current codebase, so this remains `TBD` until `UNIVERSITY_ADMIN` assignment is introduced.

The tables are defined by `backend/apps/universities/migrations/0001_initial.py` and `0002_collegecourse.py`. Test coverage: `backend/apps/universities/tests.py`. Frontend adapter and page behavior coverage: `frontend/src/services/backendUniversityService.test.ts` and `frontend/src/pages/admin/maintenance/UniversitiesListMaintenance.test.tsx`.
