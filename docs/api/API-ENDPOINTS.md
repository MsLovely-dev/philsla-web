...
- `POST /api/v1/applications/registration/identity/selfie-face/` accepts `multipart/form-data` with `file` plus `X-Registration-Token` and returns server-side live-frame face validation without storing media, including `faceDetected`, `faceCount`, `confidence`, `boundingBox`, and `faceCovered`.
- `POST /api/v1/applications/registration/identity/selfie/` accepts `multipart/form-data` with `file`, validates that exactly one centered uncovered frontal face is present server-side, and stores the image as the student's enrolled biometric reference.
- Manual Step 1 selfie capture uses frontend live detection/countdown only and does not call a backend frame-validation endpoint.
