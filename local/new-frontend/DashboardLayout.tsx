...
const SIDEBAR_GROUPS: SidebarGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: FileText, label: 'Audit Logs', href: '/admin/reviewer/audit-logs', roles: ['ADMISSIONS_REVIEWER', 'SYSTEM_ADMIN'] },
...
export function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  const filteredGroups = SIDEBAR_GROUPS.filter(group => {
    ...
    // Filter student pages for stud1resubmit so they know they must verify first
    if (group.label === 'Student Portal' && (user.id === 'student-resubmit' || user.email === 'stud1resubmit@philsa.edu.ph')) {
      items = [
        { icon: LayoutDashboard, label: 'Dashboard', href: '/dashboard' },
        { icon: ShieldCheck, label: 'Identity Verification', href: '/student/verification' }
      ];
    }
