...
export default function ExamSets() {
  // Global States
  const [activeTab, setActiveTab] = useState<'blueprints' | 'dashboard' | 'assembly' | 'packages' | 'audit'>('blueprints');
  ...
  return (
    <div className="space-y-6 text-philsa-navy max-w-7xl mx-auto px-4 md:px-0">
      ...
      <div className="flex flex-wrap gap-1 border-b border-philsa-border pb-1 select-none">
        {[
          { id: 'blueprints', label: 'Exam Blueprint', icon: ClipboardList },
          { id: 'dashboard', label: 'Exam Set Assembly', icon: Layers },
          { id: 'assembly', label: 'Exam Builder', icon: Sliders },
                    ...
                    {(() => {
                      ...
                      return list.map(bp => (
                        <option key={bp.id} value={bp.id}>
                          [{bp.status || 'APPROVED'}] {bp.code} - {bp.name}
                        </option>
                      ));
                    })()}
                  </select>
