...
export default function ProctorIncidents() {
  const { addAuditLog } = usePhilSA();
  const { schedules } = useMockData();
  const [selectedIncident, setSelectedIncident] = useState<any>(null);

  // Synchronized state with ProctorSchedule and ProctorAttendance
  const [distStates] = useState<Record<string, {
  ...
  // Pool all assigned students
  const allAssignedStudents = [
    ...getDistState('sch1').students,
    ...getDistState('sch2').students,
  ];

  const allAssignedStudentNames = allAssignedStudents.map(s => s.name.toLowerCase());
  ...
  // Helper to find the schedule details for an assigned student ID
  const getStudentSchedule = (studentId: string) => {
    const inSch1 = getInitialStudentPCs('sch1').some(s => s.id === studentId);
    const schId = inSch1 ? 'sch1' : 'sch2';
    return schedules.find(s => s.id === schId);
  };

  // Filter incidents to show ONLY those matching the proctor's assigned students
  const filteredIncidents = incidents
    .filter((i) => {
      return allAssignedStudentNames.includes(i.student.toLowerCase());
    })
    .filter((i) => {
  ...
  const handleCreateCase = (e: React.FormEvent) => {
    ...
    const targetStudent = allAssignedStudents.find(s => s.id === formStudentId);
    ...
    const studentSch = getStudentSchedule(targetStudent.id);
    const centerName = studentSch?.testCenter || "UP Diliman";
    ...
    const updatedIncidents = [newCase, ...incidents];
    setIncidents(updatedIncidents);
    addAuditLog('INCIDENT_REPORTED', `Proctor reported ${formType} incident (${newId}) for candidate ${targetStudent.name} (${targetStudent.id}) at seat ${targetStudent.seat}.`);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <AnimatePresence>
        {showSuccessToast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed top-8 right-8 z-[11000] bg-emerald-600 text-white px-6 py-4 rounded-2xl shadow-xl flex items-center gap-3 font-semibold border border-emerald-500"
          >
            <CheckCircle2 className="w-5 h-5 text-emerald-100" />
            <div>
              <p className="text-sm font-bold">Incident Logged Successfully</p>
              <p className="text-xs text-emerald-100 font-medium">
                New integrity record added and dispatched to Command Center.
              </p>
            </div>
          </motion.div>
        )}
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-philsa-gray font-medium max-w-2xl">
            Review reported examination violations and documented evidence of integrity breaches for candidates assigned to your jurisdiction.
          </p>
        </div>
      ...
      <div className="space-y-6">
        <div className="flex flex-col md:flex-row gap-4 items-center">
          ...
          <div className="flex items-center gap-2 bg-white border border-philsa-border rounded-2xl px-4 py-3 shrink-0 w-full md:w-auto">
            <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Severity:</span>
            <select
              value={severityFilter}
              onChange={(e) => setSeverityFilter(e.target.value)}
              className="bg-transparent border-none text-xs font-black text-philsa-navy uppercase tracking-wider outline-none cursor-pointer"
            >
          ...
          <div className="card-philsa text-center py-20 divide-y-0 space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-philsa-gray max-w-sm mx-auto">
                There are no reported incident records matching your criteria. All assigned students are currently system-compliant.
              </p>
            </div>
            {searchQuery && (
              <button
                    ...
                    <td className="px-10 py-7 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          ...
                          className="p-3 bg-white hover:bg-philsa-bg text-philsa-navy rounded-xl transition-colors border border-philsa-border shadow-sm cursor-pointer"
                        >
      ...
      <AnimatePresence>
        {selectedIncident && (
          <div className="fixed inset-0 z-[12000] flex items-center justify-center p-6 bg-philsa-navy/60 backdrop-blur-sm">
            <motion.div
            ...
            >
              ...
              <div className="p-10 space-y-8">
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    onClick={() => setSelectedIncident(null)}
                    className="w-full py-4 bg-philsa-bg hover:bg-slate-50 text-philsa-navy rounded-2xl text-[10px] font-black uppercase tracking-widest border border-philsa-border transition-all cursor-pointer"
                  >
                    Close Entry
                  </button>
                </div>
      ...
      <AnimatePresence>
        {isCreateModalOpen && (
          <div className="fixed inset-0 z-[12000] flex items-center justify-center p-4 bg-philsa-navy/60 backdrop-blur-sm overflow-y-auto">
            <motion.div
              ...
              <div className="px-6 py-4 border-b border-philsa-border flex justify-between items-center bg-philsa-bg/30">
                <div>
                  <h2 className="text-lg font-black text-philsa-navy tracking-tight uppercase leading-none">
                    Register <span className="text-philsa-red">New Incident</span>
                  </h2>
              ...
              >
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    className="w-full bg-philsa-bg border border-philsa-border/80 rounded-xl px-3 py-2.5 text-xs font-bold text-philsa-navy focus:ring-2 focus:ring-philsa-red/10 shadow-sm transition-all cursor-pointer outline-none"
                  >
                    <option value="">-- Choose Candidate --</option>
                    {allAssignedStudents.map((s) => (
                      <option key={s.id} value={s.id}>
                        {s.name} (ID: {s.id} | Seat: {s.seat} | Room: {getStudentSchedule(s.id)?.room || 'N/A'})
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-philsa-bg border border-philsa-border/80 rounded-xl px-3.5 py-2 text-xs font-semibold focus:ring-2 focus:ring-philsa-red/10 outline-none shadow-sm transition-all placeholder:text-philsa-gray/30 text-philsa-navy resize-none"
                  />
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <div
                    className="border border-dashed border-slate-200 hover:border-philsa-navy hover:bg-slate-50 rounded-xl p-4 text-center cursor-pointer transition-all relative"
                ...
                <div className="p-3 bg-amber-50 rounded-xl text-[9px] text-amber-800 font-semibold border border-amber-100/60 leading-normal flex items-start gap-2">
                  ...
                  <span>
                    The incident is automatically registered with your testing room's metadata.
                  </span>
                </div>
